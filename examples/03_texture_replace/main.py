...
def create_colored_images(data_path, output_data_path, base_name):
...
def generate_model_images(model_base_name, env, data_path, color_data_path,
                          generate_data_path, floor_id, wall_ids,
                          index_offset):
    ...
    obj_id = env.add_model(
        path=model_path, scale=[0.8, 0.8, 0.8], pos=[0, 1.5, -5], rot=0.0)
    ...
    for floor_color_index in range(color_size):
        # Change floor color
        floor_texture_path = "{}/floor{}.png".format(color_data_path,
                                                     floor_color_index)
        env.replace_obj_texture(floor_id, floor_texture_path)

        for wall_color_index in range(color_size):
            # Change wall color
            wall_texture_path = "{}/wall{}.png".format(color_data_path,
                                                       wall_color_index)
            ...
            for obj_color_index in range(color_size):
                # Change obj color
                obj_texture_path = "{}/{}{}.png".format(
                    color_data_path, model_base_name, obj_color_index)
                ...
                env.locate_object(obj_id, pos=[0, 1.2, distance], rot=rot)
...
def main():
    ...
    output_data_path = os.path.dirname(
        os.path.abspath(__file__)) + "/colored_data"
    ...
    # Create environment
    env = rodentia.Environment(
        width=IMAGE_SIZE, height=IMAGE_SIZE, bg_color=[0.66, 0.91, 0.98])
    ...
    generate_data_path = os.path.dirname(
        os.path.abspath(__file__)) + "/generated_data"
