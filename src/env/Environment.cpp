...
Model::Model(btDynamicsWorld* world_,
	...
	bodies[BODYPART_PELVIS] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_PELVIS]);

	// 脊柱
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(0.0), btScalar(1.0), btScalar(-0.2)));
	bodies[BODYPART_SPINE] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_SPINE]);

	// 頭
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(0.0), btScalar(1.0), btScalar(-0.6)));
	bodies[BODYPART_HEAD] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_HEAD]);

	// 左上足
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(-0.18), btScalar(0.65+0.175-0.1), btScalar(0.125)));
	bodies[BODYPART_LEFT_UPPER_LEG] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_LEFT_UPPER_LEG]);

	// 左下足
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(-0.18), btScalar(0.2+0.175-0.1), btScalar(0.125)));
	bodies[BODYPART_LEFT_LOWER_LEG] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_LEFT_LOWER_LEG]);

	// 右上足
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(0.18), btScalar(0.65+0.175-0.1), btScalar(0.125)));
	bodies[BODYPART_RIGHT_UPPER_LEG] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_RIGHT_UPPER_LEG]);

	// 右下足
	transform.setIdentity();
	transform.setOrigin(btVector3(btScalar(0.18), btScalar(0.2+0.175-0.1), btScalar(0.125)));
	bodies[BODYPART_RIGHT_LOWER_LEG] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_RIGHT_LOWER_LEG]);
	...
	bodies[BODYPART_LEFT_UPPER_ARM] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_LEFT_UPPER_ARM]);
	...
	bodies[BODYPART_LEFT_LOWER_ARM] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_LEFT_LOWER_ARM]);
	...
	bodies[BODYPART_RIGHT_UPPER_ARM] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_RIGHT_UPPER_ARM]);
	...
	bodies[BODYPART_RIGHT_LOWER_ARM] =
		createRigidBody(btScalar(1.), offset*transform, shapes[BODYPART_RIGHT_LOWER_ARM]);
	...
	hingeC = new btHingeConstraint(*bodies[BODYPART_PELVIS], *bodies[BODYPART_SPINE],
								   localA, localB);
	...
	coneC = new btConeTwistConstraint(*bodies[BODYPART_SPINE], *bodies[BODYPART_HEAD],
									  localA, localB);
	...
	coneC = new btConeTwistConstraint(*bodies[BODYPART_PELVIS], *bodies[BODYPART_LEFT_UPPER_LEG],
									  localA, localB);
	...
	hingeC = new btHingeConstraint(*bodies[BODYPART_LEFT_UPPER_LEG],
								   *bodies[BODYPART_LEFT_LOWER_LEG],
								   localA, localB);
	...
	coneC = new btConeTwistConstraint(*bodies[BODYPART_PELVIS],
									  *bodies[BODYPART_RIGHT_UPPER_LEG],
									  localA, localB);
	coneC->setLimit(M_PI_4, M_PI_4, 0);
	...
	localA.getBasis().setEulerZYX(0,M_PI_2,0); localA.setOrigin(btVector3(btScalar(0.), btScalar(-0.225), btScalar(0.)));
	localB.getBasis().setEulerZYX(0,M_PI_2,0); localB.setOrigin(btVector3(btScalar(0.), btScalar(0.185), btScalar(0.)));
	hingeC =  new btHingeConstraint(*bodies[BODYPART_RIGHT_UPPER_LEG],
									*bodies[BODYPART_RIGHT_LOWER_LEG],
									localA, localB);
	hingeC->setLimit(btScalar(0), btScalar(M_PI * 0.9));
	...
	coneC = new btConeTwistConstraint(*bodies[BODYPART_SPINE],
									  *bodies[BODYPART_LEFT_UPPER_ARM],
									  localA, localB);
	coneC->setLimit(M_PI_4, M_PI_4, 0);
	...
	hingeC =  new btHingeConstraint(*bodies[BODYPART_LEFT_UPPER_ARM],
									*bodies[BODYPART_LEFT_LOWER_ARM],
									localA, localB);
	...
	localA.getBasis().setEulerZYX(0,M_PI_2,0); localA.setOrigin(btVector3(btScalar(0.), btScalar(-0.18), btScalar(0.)));
	localB.getBasis().setEulerZYX(0,M_PI_2,0); localB.setOrigin(btVector3(btScalar(0.), btScalar(0.14), btScalar(0.)));
	hingeC =  new btHingeConstraint(*bodies[BODYPART_RIGHT_UPPER_ARM],
									*bodies[BODYPART_RIGHT_LOWER_ARM],
									localA, localB);
	hingeC->setLimit(btScalar(-M_PI_2), btScalar(0));
...
void Model::setMotorTargets(float timeUs, float deltaTimeUs) {
	...
	const float muscleStrength = 0.5f; // maxMotorImpulse
